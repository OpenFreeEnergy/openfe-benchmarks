...
def get_chemical_systems(
    ...
    for network_name, entry in exp_data.items():
        ...
        if solute_inchikey == "XLYOFNOQVPJJNP-UHFFFAOYNA-N":  # water
            ...
            solute = mol_dict[solute_inchikey]
        ...
        if solvent_inchikey != "XLYOFNOQVPJJNP-UHFFFAOYNA-N":  # water
            EXPECTED_LIGANDS.add(solvent_inchikey)
...
def get_water_settings(forcefield: str, water_model: str = "tip3p") -> ASFESettings:
    ...
    # _______________ Nondefault Settings _________________________________
    # In Alchemiscale you can set protocol_repeats to 1 and then
    ...
    settings.solvation_settings = PackmolSolvationSettings(
        box_shape="dodecahedron",  # nondefault
        assign_solvent_charges=False,
        solvent_padding=1.5 * unit.nanometer,  # nondefault
    )
    ...
    # _______________ Default Settings _________________________________
...
def get_nonwater_settings(forcefield: str) -> ASFESettings:
    ...
    forcefield : str
        Name of the force field to use.
    assign_solvent_charges : bool, default=False
        Whether to assign solvent charges or not. We expect to set them in all cases but water.
...
def compile_transformations(
    ...
    for sys_name, system_solvated_solute in sorted(systems_by_name.items()):
        # An SFE transformation in pontibus formalism is defined as
