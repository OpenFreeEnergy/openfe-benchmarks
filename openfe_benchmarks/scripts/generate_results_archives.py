...
def _combine_hybrid_topology_results(results: list[dict]) -> list[dict]:
    ...
    for paired_results in results_by_key.values():
        ...
        if (phases := set(results_by_phase.keys())) != {"solvent", "complex"}:
            raise RuntimeError(f"Only relative binding free energy calculations are currently supported with the hybrid topology found phases: {phases}")
        ...
        combined_data = {
            ...
            "ddg": results_by_phase["complex"]["estimate"] - results_by_phase["solvent"]["estimate"],
            "ddg_uncertainty": np.sqrt(results_by_phase["complex"]["estimate_error"]**2 + results_by_phase["solvent"]["estimate_error"]**2),
            "dgs_complex": results_by_phase["complex"]["individual_estimates"],
            "dgs_solvent": results_by_phase["solvent"]["individual_estimates"],
            "complex_mbar_errors": results_by_phase["complex"]["individual_mbar_errors"],
            "solvent_mbar_errors": results_by_phase["solvent"]["individual_mbar_errors"],
            "complex_smallest_mbar_overlaps": results_by_phase["complex"]["smallest_mbar_overlaps"],
...
def _extract_results_from_archive(alchemical_archive):
    ...
    Returns a dictionary keyed by the protocol class with lists of result dictionaries for each transformation.
    """
    # map the supported protocols to their extraction functions
    extraction_functions = {
        # default hybrid rbfe in openfe using a split leg protocol
    ...
    for transformation, dag_results_list in alchemical_archive.transformation_results:
        ...
        # store the results under the protocol type
        raw_results[protocol_cls].append(result_data)

    # combine outputs from the hybrid topology split leg protocols into a single result
    if RelativeHybridTopologyProtocol in  raw_results:
        raw_results[RelativeHybridTopologyProtocol] = _combine_hybrid_topology_results(raw_results[RelativeHybridTopologyProtocol])
    if HybridTopProtocol in raw_results:
        raw_results[HybridTopProtocol] = _combine_hybrid_topology_results(raw_results[HybridTopProtocol])
...
def run_generate_results(
    ...
    """Generate computational results from an alchemical archive.
    ...
    if ASFEProtocol in archive_results_by_protocol and (HybridTopProtocol in archive_results_by_protocol or RelativeHybridTopologyProtocol in archive_results_by_protocol):
        raise RuntimeError(f"Found a mix of ASFE and Hybrid Topology protocols in the archive. This is not supported. Found protocols: {list(archive_results_by_protocol.keys())}")
    ...
    if ASFEProtocol in archive_results_by_protocol:
        ...
        for result in archive_results_by_protocol[ASFEProtocol]:
            # update the system group and name
        ...
        # now build the DGs from the ddgs
        fe_map = FEMap()
...
def main(
    archive, output_dir, system_group, system_name
):
