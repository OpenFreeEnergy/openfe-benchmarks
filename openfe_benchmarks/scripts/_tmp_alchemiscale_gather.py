...
def run_gather(
    ...
    if isinstance(network, str):
        network = pathlib.Path(network)

    # we can not use scope with network/network-key
    if network_key is not None and (
        network is not None
        or org is not None
    ...
    if network is not None:
        ...
        if network_key is not None:
            ...
            logger.info(
                f"Querying Alchemiscale for submissions with scope: {query_scope}"
            )
    ...
    for network_key, network in networks_by_keys.items():
        ...
        if any(v == 0 for v in status.values()):
            if not allow_partial:
                ...
                logger.warning(
                    f"Network {network_key} has incomplete edges but will be archived anyway as --allow-partial is set."
                )

        # we need the transformations and the results to build the archive
        transformation_results = []
        alchemiscale_network_results = client.get_network_results(
            network_key, return_protocoldagresults=True
        ).items()
        for transformation_key, raw_results in alchemiscale_network_results:
            transformation = client.get_transformation(
                transformation_key, visualize=False
            )
        ...
        logger.info(
            f"Archive for network {network_key} created and saved to {output / f'{network_key}.json.bz2'}"
        )
...
@click.option(
    ...
    help="Path to the submitted alchemical network with the scope key saved as into the name field for which an archive should be made.",
    default=None,
)
@click.option(
    "--network-key",
    type=str,
    help="The scope key of the network for which an archive should be made.",
    default=None,
)
@click.option(
    "--org",
    type=str,
    help="Organization name used to build the scope to check can not be used with --network/network-key.",
    default=None,
)
@click.option(
    "--campaign",
    type=str,
    help="Campaign name used to build the scope to check can not be used with --network/network-key.",
    default=None,
)
@click.option(
    "--project",
    type=str,
    help="Project name used to build the scope to check can not be used with --network/network-key.",
    default=None,
)
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Allow archive creation for an incomplete network.",
)
@click.option(
    "--output",
    type=click.Path(dir_okay=True, path_type=pathlib.Path),
    help="Path to the output directory where the archive(s) should be made.",
    required=True,
)
def main(network, network_key, org, campaign, project, allow_partial, output):
    """CLI wrapper for run_gather.
